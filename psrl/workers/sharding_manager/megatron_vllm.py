...
class PSRL_MegatronvLLMShardingManager(BaseShardingManager):
...
class PSRL_MegatronASyncvLLMShardingManager(BaseShardingManager):
    """A sharding manager that bridges Megatron-LM training with vLLM inference.
    ...
    async def __aexit__(self, exc_type, exc_value, traceback):
        if self.rollout_config.free_cache_engine:
            await self.inference_engine.sleep(level=1)
        for model in self.actor_module:
            model.train()
