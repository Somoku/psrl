...
staleness=${1:-1}
project_name=psrl_revision_perf_70b
experiment_name=DAPO-Llama3-70B-megatron-staleness_${staleness}
fix_weight=${2:-False}
...
HF_MODEL_PATH=${PSRL_WORKSPACE}/models/DeepSeek-R1-Distill-Llama-70B
DIST_CKPT_PATH=${PSRL_WORKSPACE}/models/mcore_ckpt/DeepSeek-R1-Distill-Llama-70B
# 70B dense model: a single GPU cannot hold the full bf16 mcore model (~140GB),
# so run the conversion fully on CPU. Requires ~300GB host RAM.
python ${PSRL_PATH}/scripts/convert_hf_to_mcore.py \
...
TRAIN_PP=6 # PP in the training side 
TRAIN_CP=2 # CP in the training side
NUM_LAYERS_IN_FIRST_PIPELINE_STAGE=12 # Number of layers in the first pipeline stage
NUM_LAYERS_IN_LAST_PIPELINE_STAGE=12 # Number of layers in the last pipeline stage
...
TRAIN_NNODES=12 # Number of nodes for training
