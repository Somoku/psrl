...
class GroupPostProcessorRegistry:
    ...
    def register(cls, name: str):
        ...
        """
        def decorator(processor_cls: Type[BaseGroupPostProcessor]) -> Type[BaseGroupPostProcessor]:
    ...
    def get(cls, name: str) -> Type[BaseGroupPostProcessor]:
        ...
        if name not in cls._registry:
            raise KeyError(f"Group post-processor '{name}' is not registered. "
                          f"Available processors: {list(cls._registry.keys())}")
...
class BufferPostProcessorRegistry:
    ...
    def register(cls, name: str):
        ...
        Returns:
            The decorator function.
        """
        def decorator(processor_cls: Type[BaseBufferPostProcessor]) -> Type[BaseBufferPostProcessor]:
            if name in cls._registry:
    ...
    def get(cls, name: str) -> Type[BaseBufferPostProcessor]:
        ...
        """
        if name not in cls._registry:
            raise KeyError(f"Buffer post-processor '{name}' is not registered. "
                          f"Available processors: {list(cls._registry.keys())}")
        return cls._registry[name]
...
def load_group_post_processor(config: DictConfig) -> Optional[BaseGroupPostProcessor]:
    """
    ...
    try:
        ...
        raise ValueError(
            f"Unknown group post-processor '{processor_name}'. "
            f"Available processors: {available_processors}"
        ) from e

def load_buffer_post_processor(config: DictConfig) -> Optional[BaseBufferPostProcessor]:
    """
    ...
    try:
        ...
        raise ValueError(
            f"Unknown buffer post-processor '{processor_name}'. "
            f"Available processors: {available_processors}"
        ) from e
