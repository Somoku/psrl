...
# ---------------------------------------------------------------------------
# Protocol decorators — purely informational, no runtime behavior change.
# ---------------------------------------------------------------------------


def whitebox_protocol(func):
    """
    Mark a method as part of the white-box agent protocol.

    White-box methods are used when PSRL controls the interaction loop
    (e.g. tool-based multi-turn). The agent loop explicitly calls
    ``update_from_env`` / ``update_from_model_*`` / ``prepare_generation_request``
    to drive each step. Subclasses: ``ToolAgentData``.
    """
    func._agent_protocol = "whitebox"
    return func


def blackbox_protocol(func):
    """
    Mark a method as part of the black-box agent protocol.

    Black-box methods are used when an external agent controls the interaction
    loop (e.g. mini-swe-agent running in-process). PSRL only intercepts
    ``model.query()`` calls and records complete turns via ``record_turn`` /
    ``build_gen_request``. Subclasses: ``MiniSWEAgentData``.
    """
    func._agent_protocol = "blackbox"
    return func
...
class AgentData(ABC, Generic[ObsType, ActType]):
    ...
    def prepare_generation_request(
        ...
        Used by both white-box and black-box protocols:
        - White-box (no ``prompt_ids``): reads token IDs from
          ``trajectory.prompt_ids + trajectory.response_ids``.
        - Black-box (``prompt_ids`` provided): uses the externally tokenized IDs
          (e.g. from mini-swe-agent's message history).

        Args:
            request (DataProto): Original input request.
            prompt_ids (list[int] | None): Explicit prompt token IDs. When None,
                falls back to the trajectory-accumulated IDs (white-box mode).
    ...
    # --- Black-box protocol methods ---
    # These are used when an external agent (e.g. mini-swe-agent) controls the
    # interaction loop. PSRL intercepts model.query() calls and records complete
    # turns. Subclasses implementing the black-box protocol should override these
    # and raise NotImplementedError for the white-box abstract methods above.

    @blackbox_protocol
    def update_from_external_turn(self, *args, **kwargs) -> None:
        """
        Record one complete interaction turn from an external agent.

        Black-box counterpart to the white-box ``update_from_model_token_ids`` +
        ``update_from_env`` pair. Subclasses define the concrete signature.
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} must implement update_from_external_turn "
            "for black-box protocol."
        )

    @blackbox_protocol
    def encode_messages(self, *args, **kwargs) -> list[int]:
        """
        Tokenize a message list into token IDs.

        Black-box counterpart to ``encode_observation``. Subclasses define the
        concrete signature.
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} must implement encode_messages "
            "for black-box protocol."
        )
