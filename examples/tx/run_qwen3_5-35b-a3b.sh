...
# Avoid excessive idle processes.
export RAY_prestart_worker_first_driver=false
export RAY_num_workers_soft_limit=0
# Disable the Raylet OOM killer.
export RAY_memory_monitor_refresh_ms=0
# Reclaim memory when 10 percent remains.
...
python3 -m psrl.trainer.main_ppo --config-path=./config \
    ...
    train_actor_rollout_ref.actor.ppo_max_token_len_per_gpu=30720 \
    ...
    +train_actor_rollout_ref.actor.megatron.override_transformer_config.moe_aux_loss_coeff=0.01 \
    +train_actor_rollout_ref.actor.megatron.override_transformer_config.moe_z_loss_coeff=0.001 \
    ...
    train_actor_rollout_ref.rollout.log_prob_max_token_len_per_gpu=30720 \
    train_actor_rollout_ref.rollout.gpu_memory_utilization=0.4 \
    ...
    train_actor_rollout_ref.ref.log_prob_max_token_len_per_gpu=30720 \
    ...
    trainer.total_epochs=15 $@ 2>&1 | tee "train_qwen3_5moe_4.log"
