...
class GPUWorker:
    ...
    def allocate_memory(self):
        ...
        # 分配显存（每个元素4字节）
        tensor = torch.zeros(self.size // 4, dtype=torch.float32, device=device)
...
# 打印结果
for res in results:
    print(f"Actor {res['id']}: Allocated {res['allocated_mb']:.2f} MB, "
          f"GPU Total: {res['max_memory_mb']:.2f} MB")
