...
# Excised routines already name their file and subroutine in the instruction, so a
# location hint tells them nothing. Their cost is writing the body, not finding it.
# Injected and semantic defects both localize to a real file, so both are hinted:
# a semantic defect swaps one calibrated constant for another, which is as findable
# as an injected edit. `athena-gr` labels its semantic round `semantic-gr-sol`.
...
def _resolve_tasks_root(repo: Path, env: str) -> Path:
    """
    Return the task tree Harbor can actually run, preferring the compiled one.
    ...
    `laps` is already compiled in place, so its `envs/` tree is the runnable one and no
    `build/` directory exists. Preferring `build/` when present and falling back
    otherwise covers both without the caller having to know which env is which.
    ...
    if (compiled / "index.jsonl").exists():
        psrl_logger.info(f"Using compiled task tree: {compiled!s}")
        return compiled
    authored = repo / "envs" / env / "tasks"
    if not authored.is_dir():
        raise FileNotFoundError(f"Tasks root not found: {authored}, and no compiled tree at {compiled}")
    # A task with no `environment/` cannot be built, so say so here rather than letting
    # every episode fail inside Harbor.
    uncompiled = [
        p.parent.name for p in sorted(authored.glob("**/task.toml")) if not (p.parent / "environment").is_dir()
    ]
    if uncompiled:
        raise FileNotFoundError(
            f"{len(uncompiled)} tasks under {authored} have no `environment/` directory, so "
            f"Harbor cannot build them (for example {uncompiled[0]}). Compile the env first: "
            f"python utils/harbor/to_harbor.py --env envs/{env}"
        )
    psrl_logger.info(f"Using in-place compiled task tree: {authored!s}")
    return authored
...
def _load_canonical_rows(repo: Path, env: str) -> dict[str, dict[str, Any]]:
    ...
    This is the per-task canonical row. Older envs also ship an aggregated
    `tasks.jsonl`, but the per-task file is the format every env has, so reading it
    keeps one code path across `laps`, `mitgcm-biogeo`, and `athena-gr`. Verified on
    laps: `candidate` and `funnel.floor` are byte-identical between the two sources
    for all 143 tasks, and the only field `tasks.jsonl` adds is a top-level `checks`
    list that `task.toml`'s `affected_checks` reproduces exactly for all 143.

    Hand-authored tasks such as the laps acceleration pair have no provenance file
    and are simply absent, which `_build_row` already tolerates.
...
def _discover_task_dirs(
    ...
    Two layouts are supported, because they coexist in the repo. `laps` puts task
    directories directly under the category, while `mitgcm-biogeo` and `athena-gr`
    insert a difficulty tier (`tasks/repair/easy/<task>`). Recursing for `task.toml`
    handles both without asking the caller which env is which.
    ...
    Args:
        ...
        difficulty (str | None): Keep only tasks whose `metadata.taxonomy.difficulty`
            matches. None keeps every tier, which is what `laps` needs because its
            tasks carry no difficulty field at all.
    ...
    for category_dir in sorted(p for p in tasks_root.iterdir() if p.is_dir()):
        ...
        if categories is not None and category not in categories:
            continue
        # Recurse so a tier directory between the category and the task is transparent.
...
def _build_hint(
    ...
    if level == "L1" and line is not None and len(defect_files) == 1:
        ...
        # 35 athena-gr tasks repeat one edit across all six relativistic Riemann
        # solvers, and listing them costs 416 chars against a 400 budget. Naming the
        # shared directory stays inside the budget and localizes just as well, because
        # the instruction is that every file under it received the same edit.
...
def _build_row(
    ...
    # Harbor re-reads `instruction.md` from `task_path` and appends the hint itself,
    # so this column is a faithful record of the delivered prompt rather than the
    # delivery path. Keeping the two in step lets offline eval and inspection read it.
...
def _interleave_categories(df: pd.DataFrame, seed: int = 20260828) -> pd.DataFrame:
    ...
    `_discover_task_dirs` walks category directories in order, so the frame
    arrives grouped: every `implementation` row precedes every `repair` row. A
    sequential sampler then spends the first steps of a run entirely inside one
    category, which is how a 16 wide batch saw only restore tasks and never a
    hinted repair task. Shuffling here makes the artifact correct on its own
    rather than relying on `data.shuffle` being set downstream.
...
def build_datasets(
    ...
    """
    Build the train split for one hint level, plus the shared unhinted validation set.

    Train and validation are built from separate row passes. The validation set is
    always unhinted so scores stay comparable across levels and against the
    standalone eval, while the train split carries whichever hint `hint_level` asks
    for. The split itself is chosen on the unhinted frame, so every level trains and
    ...
    Args:
        ...
        difficulty (str | None): Keep only this `metadata.taxonomy.difficulty` tier.
            None keeps every tier, which `laps` requires because its tasks predate the
            field.
    ...
    df.to_parquet(out / f"{hint_level}_all.parquet", index=False)
    train_df.to_parquet(out / f"{hint_level}_train.parquet", index=False)
    # One validation set serves every level, so writing it repeatedly is idempotent.
    val_df.to_parquet(out / "val.parquet", index=False)
    # The hinted validation split, held on the same task partition. `val.parquet` stays
    # unhinted so a run can measure unaided localization, but a model trained on hinted
    # prompts sees a different task there and scores near zero for the wrong reason.
    # Evaluating in-distribution needs the hint the training split carried. `L3` is the
    # unhinted control by construction, so it would only duplicate `val.parquet`.
    if hint_level != "L3":
        hinted_val_df = df[df["task_name"].isin(val_names)].reset_index(drop=True)
        hinted_val_df.to_parquet(out / f"{hint_level}_val.parquet", index=False)
    ...
    (out / f"{hint_level}_stats.json").write_text(json.dumps(stats, indent=2), encoding="utf-8")

    psrl_logger.info(
        f"Wrote dataset={out / f'{hint_level}_all.parquet'!s}. Total tasks: {len(df)}. "
        f"Hinted: {stats['n_hinted']}. Train: {len(train_df)}. Validation: {len(val_df)} (unhinted). "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Keep only this metadata.taxonomy.difficulty tier, e.g. 'easy'. Omit for "
            "envs whose tasks carry no difficulty field, such as laps."
        ),
    )
