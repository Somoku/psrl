...
from verl.trainer.ppo.ray_trainer import AdvantageEstimator, ResourcePoolManager, apply_kl_penalty, compute_response_mask, compute_advantage, _timer, RayPPOTrainer
...
class PSRL_RayPPOTrainer(RayPPOTrainer):
    ...
    def init_workers(self):
        ...
        assert not self.hybrid_engine, "hybrid engine is not compatible with PSRL"
        ...
        rollout_resource_pool = self.resource_pool_manager.get_resource_pool(PSRL_Role.Rollout)
        ...
        ps_cls = RayClassWithInitArgs(
            ...
            config=self.config.parameter_server,
            role='ps',
            reward_config=self.config.reward_model,
        )
        self.resource_pool_to_cls[actor_resource_pool]['actor'] = actor_cls
