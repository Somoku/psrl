...
class Trajectory:
    ...
    routed_experts: np.ndarray | torch.Tensor | None = None
...
class AgentData(ABC, Generic[ObsType, ActType]):
    ...
    async def finalize_output(self) -> TokenOutput | list[TokenOutput] | None:
        ...
        for trajectory in self.session_data.trajectories:
            ...
            routed_experts = None
            if trajectory.routed_experts is not None and trajectory.response_ids:
                expected_rows = len(trajectory.prompt_ids) + len(trajectory.response_ids) - 1
                if len(trajectory.routed_experts) < expected_rows:
                    raise ValueError(
                        "routed_experts is shorter than the finalized trajectory: "
                        f"got {len(trajectory.routed_experts)} rows, expected {expected_rows}."
                    )
                routed_experts = trajectory.routed_experts[:expected_rows]
