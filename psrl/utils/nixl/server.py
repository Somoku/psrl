...
class NIXLMetaServer:
    ...
    def __init__(self, server_name: str, nixl_config: DictConfig):
        ...
        self.connected_clients: Dict[str, List[str]] = {} # agent_name -> [client_name1, client_name2, ...], one agent can bind to multiple clients
            ...
            for agent_name, notif_list in notifs.items():
                for notif in notif_list:
                    try:
                        ...
                        assert isinstance(multi_shardings, dict), f"Expected a dict of multi_shardings, but got {multi_shardings}"
    ...
    def wait_for_client_temp_mappings(self, expected_agents: int = 1, timeout: float = 360.0):
            ...
            for agent_name, notif_list in notifs.items():
                for notif in notif_list:
                    try:
                        ...
                        assert isinstance(multi_temp_mappings, dict), f"Expected a dict of multi_temp_mappings, but got {multi_temp_mappings}"
        ...
        psrl_logger.info(f"All {len(self._client_temp_mappings)} clients of {expected_agents} agents sent temp mappings after {time.time() - start} seconds.")
    ...
    def notify_all_client_shardings(self):
        ...
        for agent_name, client_names in self.connected_clients.items():
            ...
            for client_name in client_names:
                assert client_name in self.client_unified_sharding_dicts, f"Client {client_name} not found in unified sharding dicts."
