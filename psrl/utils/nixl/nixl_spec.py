...
class NIXLSharding:
    ...
    def _validate_and_set_local_shard_mesh(self) -> None:
        ...
        for idx, tup in enumerate(self.shard_indices):
            if len(tup) != n_dim:
                raise ValueError(
                    f"All tuples must have the same length. "
                    f"Found {len(tup)} at index {idx}, expected {n_dim}."
                )
        ...
        for i in range(1, len(self.shard_indices)):
            ...
            if prev >= curr:
                raise ValueError(
                    "Shard indices must be in strictly increasing order. "
                    f"At index {i}: {prev} >= {curr}."
                )
        ...
        self._local_shard_mesh = OrderedDict((actual_dim, size) for actual_dim, size in zip(self.shard_mesh.keys(), dim_sizes))
    ...
    @staticmethod
    def find_finest_shard_mesh(shard_mesh_list: List[OrderedDict[int, int]]) -> OrderedDict[int, int]:
    ...
    def _calculate_scaling_factors(self, finer_shard_mesh: OrderedDict[int, int]) -> Dict[int, int]:
        ...
        for dim, current_shard_mesh in self.shard_mesh.items():
            if dim in finer_shard_mesh:
                ...
                assert current_shard_mesh == 1, "Current dimension doesn't exist in finer sharding, and current shard_mesh is not 1"
    ...
    def _generate_new_shard_indices(
        ...
        for current_shard_idx_tuple in self.shard_indices:
            ...
            current_dim_to_idx = {}
            assert len(self.shard_mesh) == len(current_shard_idx_tuple), "Current shard index tuple must have the same length as shard_mesh"
    ...
    def _expand_recursive(
        ...
        if current_dim in self.shard_mesh:
            ...
            for idx in range(start_idx, end_idx):
                ...
                self._expand_recursive(
                    current_dim_to_idx, scaling_factors, finer_shard_mesh,
                    dim_idx + 1, new_combination, result
                )
        else:
            # New dimension, generate all possible indices
            scaling_factor = scaling_factors[current_dim]  
            assert scaling_factor == finer_shard_mesh[current_dim], "When it's a new dimension, scaling factor must be equal to the number of shards in the new dimension"
...
class NIXLTensorInfo:
    ...
    def get_shard_size_bytes(self, local_pos: int) -> int:
        ...
        assert local_pos < self.num_local_shards, f"Shard position {local_pos} is out of range for tensor with {self.num_local_shards} shards"
