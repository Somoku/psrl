...
class PSRL_BaseTrainWorker(ABC):
    ...
    def nixl_push_model(self) -> None:
        ...
        psrl_logger.debug(f"Getting the current PS model version...")
        ...
        psrl_logger.debug(f"Pushing the model with version {next_ps_model_version} to the PS via NIXL on {len(self._cached_ps_nixl_train_storage_client_names)} clients.")
            ...
            try:
                ...
                for key in self.unified_state_dict:
                    ...
                    for target_agent_name, target_client_name in zip(self._cached_ps_nixl_agent_names, self._cached_ps_nixl_train_storage_client_names): 
                        ...
                        psrl_logger.debug(f"Pushing key {key} to {target_client_name} for version {next_ps_model_version}")
                        try:
                            ...
                            # shards_to_transfer = self.nixl_storage_client.client_write(target_agent_name, target_client_name, key, "train_push")
                    ...
                    psrl_logger.debug(f"Starting to wait for {len(wait_operations)} NIXL operations for version {next_ps_model_version}...")
                    for key, target_client_name, shards_to_transfer in wait_operations:
                        try:
                            ...
                            # self.nixl_storage_client.wait(key, "train_push", "WRITE", target_client=target_client_name)
                        except Exception as e:
                ...
                dist.barrier()
                psrl_logger.debug(f"Barrier done, now pushing model tag to the PS on the representative rank...")
