...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        print(f"inside verl: ckpt1")
        if loop.is_running():
            tasks = asyncio.all_tasks(loop)
            print(f"inside verl: {tasks=}")
        ...
        print(f"inside verl: {dp=}, {infer_tp=}")
    ...
    def init_model(self):
        ...
        print(f"inside verl: ckpt-2")
        if loop.is_running():
            tasks = asyncio.all_tasks(loop)
            print(f"inside verl: {tasks=}")

        if self._is_rollout:
            print(f"inside verl: is_rollout")
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if self._is_actor:  # For rollout only, we do not switch context.
            loop = asyncio.get_event_loop()
            loop.call_soon_threadsafe(asyncio.ensure_future, self.trainer_mode())
            # loop.run_until_complete(self.rollout_mode())
            log_gpu_memory_usage("After switch to rollout mode", logger=logger)

        print(f"inside verl: {prompts=}")
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        loop = asyncio.get_event_loop()
        print(f"inside verl: ckpt critic")
        if loop.is_running():
            tasks = asyncio.all_tasks(loop)
            print(f"inside verl: {tasks=}")
        # This is used to import external_lib into the huggingface systems
