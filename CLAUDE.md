...
Principle 2: When running scripts or inspecting the environment, please activate the conda environment by executing `source /jizhicfs/lhy/env/psrl.sh`. All dependencies and packages are installed within this environment. For basic bash commands such as `grep`, `ls`, `find`, and `read`, you could execute them directly without asking for my permission.
