...
Usage:
    ...
    # `per_rank_torch_save` format only. `per_rank_plain_tensors` lacks
    # ShardedBase metadata and cannot be converted to DCP.
"""
...
def main():
    ...
    assert os.path.exists(metadata_path), (
        f"Metadata path={metadata_path!r} does not exist. "
        f"Input directory={args.input_dir!r} is not a valid per-rank checkpoint."
    )
    ...
    assert saved_ws == world_size, (
        ...
        f"Required process count={saved_ws}. Run torchrun with matching --nproc_per_node and --nnodes values."
    )
    ...
    save_strategy = FullyParallelSaveStrategyWrapper(
        ...
        parallelization_group=None,  # coordinate shard assignment across all ranks
        do_cache_distribution=False,  # one-shot conversion, no need to cache
    )

    # NOTE(lhy): Forked DCP writers inherit live NCCL handles and corrupt the parent
    # communicator. Run write buckets sequentially in the parent process.
    ...
    def _write_preloaded_data_multiproc_nofork(transform_list, use_msc, rank, write_buckets, global_results_queue):
        """
        Write each bucket sequentially without forking.

        A fork would inherit live NCCL state from the parent process.
        """
        write_results_or_exc = {}
        for i, write_bucket in enumerate(write_buckets):
            # The writer reports through `results_q` and waits on `count_q` before returning.
