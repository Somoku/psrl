...
class DockerEngine(Protocol):
    ...
    async def list_images(self) -> list[Mapping[str, Any]]: ...
...
class DockerEngineClient:
    ...
    async def tag_image(self, reference: str, repository: str, tag: str) -> None:
        ...
        """
        path = f"/images/{quote(reference, safe='')}/tag?repo={quote(repository, safe='')}&tag={quote(tag, safe='')}"
        await self._request("POST", path, expected=(201,))

    async def push_image(self, reference: str, tag: str, auth: Mapping[str, str] | None = None) -> None:
        """Push a tagged image to its registry and fail on a reported error.

        A push reports progress inside an HTTP 200 stream and can fail within it,
        so a status check alone would accept a partial upload as a published
        snapshot.
        """
        headers = None
        if auth:
        ...
        name = quote(reference, safe="")
        session = await self._get_stream_session()
        async with session.post(
            f"{self._base_url}/images/{name}/push?tag={quote(tag, safe='')}",
            headers=headers,
        ) as response:
            if response.status != 200:
                raise DockerEngineError(response.status, (await response.text()).strip())
    ...
    async def image_digests(self, reference: str) -> list[str]:
        ...
        Read after a push, because the digest is what makes a snapshot restorable
        from another node, and a tag can be moved.
        """
        _, body, _ = await self._request("GET", f"/images/{quote(reference, safe='')}/json", expected=(200,))
        payload = json.loads(body)
        return [str(digest) for digest in (payload.get("RepoDigests") or []) if digest]

    async def list_images(self) -> list[Mapping[str, Any]]:
        """
        List the daemon's local images, which is what an image locality index is built from.
        """
        _, body, _ = await self._request("GET", "/images/json", expected=(200,))
