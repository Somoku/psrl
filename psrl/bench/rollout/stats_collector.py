...
class StatCollector(StatLoggerBase):
    ...
    def __init__(
        ...
        # Create rank-specific log file with timestamp
        ...
        if scheduler_stats is not None:
            ...
            log_entry = {
                ...
                "iteration_stats": {
                    ...
                    "num_generation_tokens": getattr(iteration_stats, 'num_generation_tokens', 0) if iteration_stats else 0,
                    "num_preempted_reqs": getattr(iteration_stats, 'num_preempted_reqs', 0) if iteration_stats else 0,
                    "num_finished_requests": len(getattr(iteration_stats, 'finished_requests', [])) if iteration_stats else 0,
                    # "time_to_first_tokens": getattr(iteration_stats, 'time_to_first_tokens_iter', []) if iteration_stats else [],
                    # "inter_token_latencies": getattr(iteration_stats, 'inter_token_latencies_iter', []) if iteration_stats else [],
    ...
    def log_engine_initialized(self):
        ...
        # Safely get cache config values
        cache_config = getattr(self.vllm_config, 'cache_config', None)
        num_gpu_blocks = getattr(cache_config, 'num_gpu_blocks', None) if cache_config else None
        ...
        cache_info = {
            ...
            "cache_config": {
                ...
                "gpu_memory_utilization": getattr(cache_config, 'gpu_memory_utilization', None) if cache_config else None,
            }
        }
    ...
    def log_generation_end(self, request_id: str, generated_length: int, 
                          finish_reason: str, generation_time: float):
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        total_preempts = sum(entry["iteration_stats"]["num_preempted_reqs"] 
                           for entry in self.stats_history 
                           if "iteration_stats" in entry)
        
        avg_waiting = np.mean([entry["scheduler_stats"]["num_waiting_reqs"] 
                              for entry in self.stats_history])
        max_waiting = np.max([entry["scheduler_stats"]["num_waiting_reqs"] 
                              for entry in self.stats_history])
        avg_running = np.mean([entry["scheduler_stats"]["num_running_reqs"] 
                              for entry in self.stats_history])
        max_running = np.max([entry["scheduler_stats"]["num_running_reqs"] 
                              for entry in self.stats_history])
        avg_kv_cache_usage = np.mean([entry["scheduler_stats"]["kv_cache_usage"] 
                                     for entry in self.stats_history])
        max_kv_cache_usage = np.max([entry["scheduler_stats"]["kv_cache_usage"] 
                                     for entry in self.stats_history])
        
        # Calculate throughput statistics
        prompt_throughputs = [entry["throughput_stats"]["prompt_throughput"] 
                             for entry in self.stats_history 
        ...
        hit_rates = [entry["prefix_cache_stats"]["hit_rate"] 
                    for entry in self.stats_history 
                    if "prefix_cache_stats" in entry]
