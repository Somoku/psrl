...
class TestResolveChunkSize:
    ...
    def test_mini_batch_multiplier_1(self):
        ...
        cfg, dp = _make_config(granularity="mini_batch", multiplier=1,
                                train_batch_size=64, rollout_n=4, mini_batch=32)
        gran, chunk_groups = resolve_fine_grain_chunk_size(cfg, dp)
    ...
    def test_micro_batch_clamps_to_mini_batch(self):
        ...
        cfg, dp = _make_config(granularity="micro_batch", multiplier=10,
                                micro_batch_per_gpu=4, dp_size=4,
                                rollout_n=4, mini_batch=32)
        gran, chunk_groups = resolve_fine_grain_chunk_size(cfg, dp)
        assert gran == "mini_batch"
    ...
    def test_mini_batch_clamps_to_full_batch(self):
        ...
        cfg, dp = _make_config(granularity="mini_batch", multiplier=100,
                                train_batch_size=64, rollout_n=4, mini_batch=32)
        gran, chunk_groups = resolve_fine_grain_chunk_size(cfg, dp)
        assert gran == "none"
