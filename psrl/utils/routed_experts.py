...
def validate_routed_experts_array(routed_experts: Any) -> np.ndarray:
    ...
    if array.size:
        max_expert_id = int(array.max())
        if max_expert_id > _MAX_EXPERT_ID:
            raise ValueError(
                "routed_experts contains an expert id that cannot be represented by veRL's int16 replay tensor: "
                f"max={max_expert_id}, limit={_MAX_EXPERT_ID}."
            )
...
def canonicalize_routed_experts(
    ...
    Compact inference payloads omit the final sampled token because that token is
    never forwarded through the model. The canonical representation restores that
    one row as zero filler. No other missing, overlapping, or extra positions are
    accepted.
    """
    ...
    if prompt_length < 0 or response_length <= 0:
        raise ValueError(
            "Canonical routed_experts requires a non-negative prompt length and a positive response length, "
            f"got prompt_length={prompt_length}, response_length={response_length}."
        )
    ...
    if rows not in (sequence_length - 1, sequence_length):
        raise ValueError(
            "routed_experts must cover every forwarded prompt/response position exactly: "
