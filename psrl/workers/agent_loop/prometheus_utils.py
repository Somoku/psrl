# Adapted from verl/experimental/agent_loop/prometheus_utils.py
import logging
import os
...
from psrl.workers.config.rollout import PrometheusConfig
...
def update_prometheus_config(config: PrometheusConfig, server_addresses: list[str], rollout_name: str | None = None):
    ...
    if not server_addresses:
        psrl_logger.warning("No server addresses available to update Prometheus config")
        return
    ...
    try:
        ...
        psrl_logger.error(f"Failed to update Prometheus configuration: {e}")
