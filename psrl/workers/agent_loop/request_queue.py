...
def get_priority_by_version(request: DataProto, staleness: int) -> int:
    ...
    if "version_tag" in request.non_tensor_batch:
        if request.non_tensor_batch["version_tag"][0] == -1:
            # Means that the request is not allocated a version tag yet (new request that is not routed yet when enabled dynamic version tag)
...
class PriorityRequestQueue:
    ...
    def filter_by_condition(self, condition: Callable[[DataProto], bool], guarantee_order: bool = True) -> List[DataProto]:
...
class MultiPriorityRequestQueue:
    ...
    def iter_queues(self) -> Iterator[Tuple[int, PriorityRequestQueue]]:
        ...
        for queue_id, queue in self._queues.items():
            yield (queue_id, queue)
    
    def iter_all_requests(self, queue_order: Optional[List[int]] = None) -> Iterator[Tuple[int, DataProto]]:
